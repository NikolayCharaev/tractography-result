...
/** Изображения из public/prostate/17.07: `new` — Ronix, `old` — RT. */
function prostate1707Images(variant: "new" | "old", studyId: number): StudyImages {
  return {
    kind: "overview",
    src: `/prostate/17.07/${variant}/${studyId}/result_lesion_slices.png`,
    kineticCurvesSrc: `/prostate/17.07/${variant}/${studyId}/result_overview.png`,
  }
}
...
function makeRonixStudy(id: number): Study {
  return {
    ...
    images: prostate1707Images("new", id),
...
function makeRtStudy(id: number): Study {
  return {
    ...
    images: prostate1707Images("old", id),
    imagesPanelTitle: "Срезы с очагами и кинетика",
  }
}

export const report1707: Report = {
  id: "17-07",
  label: "Результаты (17.07)",
